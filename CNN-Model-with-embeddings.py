...
with open(path + 'data/PyCommitsWithDiffs_open_redirect.json', 'r') as infile:
...
# creating the model
model = Sequential()
model.add(Conv1D(128, 3, strides=5, padding='same', activation='relu'))
...
now = datetime.now()
nowformat = now.strftime("%H:%M")
print("saving CNN model "  ". ", nowformat)
model.save(path + 'model_codebert/CNN_model.h5')
print("\n\n")
